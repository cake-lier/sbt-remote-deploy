...
lazy val root = (project in file("."))
  ...
  .settings(
    ...
    initTask := {
      ...
      docker
        .pullImageCmd("matteocastellucci3/sbt-remote-deploy-unenc-keyfile")
        .start()
        .awaitCompletion()
      println("Creating container...")
      val portBindings = new Ports()
      portBindings.bind(ExposedPort.tcp(22), Binding.bindPort(2024))
      docker
        .createContainerCmd("matteocastellucci3/sbt-remote-deploy-unenc-keyfile")
        .withHostConfig(new HostConfig().withPortBindings(portBindings))
        .withName("sbt-remote-deploy-unenc-keyfile")
        .exec()
      println("Starting container...")
      docker.startContainerCmd("sbt-remote-deploy-unenc-keyfile").exec()
    },
    ...
    teardownTask := {
      ...
      docker.stopContainerCmd("sbt-remote-deploy-unenc-keyfile").exec()
      println("Deleting container...")
      docker.removeContainerCmd("sbt-remote-deploy-unenc-keyfile").exec()
      println("Deleting image...")
      docker.removeImageCmd("matteocastellucci3/sbt-remote-deploy-unenc-keyfile").exec()
    }
  )
